    ...
    <nav className="sticky top-0 z-50 bg-white/95 dark:bg-[#0a0a0a]/95 backdrop-blur-xl border-b border-gray-100 dark:border-gray-900 h-20 transition-all">
      <div className="max-w-7xl mx-auto px-4 h-full flex justify-between items-center relative">
        ...
        <Link href="/" className="flex items-center gap-2 z-50 hover:opacity-80 transition-opacity">
          ...
          <span className={`font-black text-xl md:text-2xl tracking-tighter ${activeText}`}>McWallet</span>
        </Link>
        ...
        <div className="flex items-center gap-3 md:gap-4 z-50">
          <button onClick={toggleRole} className="bg-gray-100 dark:bg-gray-800 rounded-full p-1 shadow-inner flex cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors active:scale-95" aria-label="Cambiar Rol">
            <span className={`px-3 py-1.5 rounded-full text-[10px] md:text-[11px] font-black transition-all duration-300 ${role === 'CREW' ? 'bg-white dark:bg-gray-900 shadow-sm text-blue-600 dark:text-blue-400 scale-100' : 'text-gray-400 dark:text-gray-500 scale-95'}`}>CREW</span>
            <span className={`px-3 py-1.5 rounded-full text-[10px] md:text-[11px] font-black transition-all duration-300 ${role === 'ENTRENADOR' ? 'bg-white dark:bg-gray-900 shadow-sm text-red-600 dark:text-red-400 scale-100' : 'text-gray-400 dark:text-gray-500 scale-95'}`}>ENTR</span>
          </button>
          ...
          <div className="flex items-center">
            <SignedOut>
              <SignInButton mode="modal">
                <button className="font-black text-[10px] md:text-xs uppercase tracking-widest bg-black dark:bg-white text-white dark:text-black px-4 py-2.5 rounded-xl hover:bg-gray-800 dark:hover:bg-gray-200 hover:scale-105 transition-all shadow-lg shadow-black/20">Ingresar</button>
              </SignInButton>
            </SignedOut>
            <SignedIn>
          ...
          <button onClick={() => setIsOpen(!isOpen)} className="md:hidden p-2 -mr-2 text-gray-800 dark:text-white rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 active:bg-gray-100 transition-colors focus:outline-none">
            {isOpen ? (
              ...
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-7 h-7"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 9h16.5m-16.5 6.75h16.5" /></svg>
            )}
          </button>
        </div>
      </div>
